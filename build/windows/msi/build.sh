...
APP_NAME_FOR_SETUP="${APP_NAME:-VSCodium}"

if [[ "${VSCODE_QUALITY}" == "insider" ]]; then
	PRODUCT_NAME="${APP_NAME_FOR_SETUP} - Insiders"
  ...
  SETUP_RESOURCES_DIR=".\\resources\\insider"
	...
	PRODUCT_NAME="${APP_NAME_FOR_SETUP}"
...
if [[ -z "${1}" ]]; then
	OUTPUT_BASE_FILENAME="${APP_NAME_FOR_SETUP}-${VSCODE_ARCH}-${RELEASE_VERSION}"
else
	OUTPUT_BASE_FILENAME="${APP_NAME_FOR_SETUP}-${VSCODE_ARCH}-${1}-${RELEASE_VERSION}"
fi
